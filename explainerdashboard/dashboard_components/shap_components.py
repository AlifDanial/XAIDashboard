...
class ShapSummaryComponent(ExplainerComponent):
    ...
    def __init__(self, explainer, title='Shap Summary', name=None,
                    subtitle="Ordering features by Impact value",
        ...
        The shap summary summarizes the Impact values per feature.
        You can either select an aggregates display that shows mean absolute Impact value
        per feature. Or get a more detailed look at the spread of Impact values per
                        ...
                        dbc.Col([
                            dbc.Row(
                                [
                                    ...
                                    dbc.Tooltip("Display mean absolute Impact value per feature (aggregate)"
                                                " or display every single Impact value per feature (detailed)", 
...
class ShapDependenceComponent(ExplainerComponent):
    ...
    def __init__(self, explainer, title='Shap Dependence', name=None,
        ...
        if self.description is None: self.description = """
        This plot shows the relation between column values and impact values.
        This allows you to investigate the general relationship between column
        value and impact on the prediction. You can check whether the model
        uses columns in line with your intuitions, or use the plots to learn
        about the relationships that the model has learned between the input columns
        ...
        return dbc.Card([
            ...
            dbc.CardBody([
                ...
                dbc.Row([
                    ...
                    make_hideable(
                        dbc.Col([
                            dbc.Label('Column:', id='shap-dependence-col-label-'+self.name),
                            dbc.Tooltip("Select column to display impact dependence for", 
                    ...
                    make_hideable(dbc.Col([
                            dbc.Label('Color :', id='shap-dependence-color-col-label-'+self.name),
                            dbc.Tooltip("Select column to color the scatter markers by. This "
                ...
                dbc.CardFooter([
                    dbc.Row([
                        make_hideable(
                            dbc.Col([
                                dbc.Row([
                                    dbc.Tooltip("Remove outliers in column (and color column) from plot.",
                            ...
                            dbc.Col([
                                html.Div([
                                    ...
                                    dbc.Tooltip("How to sort the categories: Alphabetically, most common "
                                                "first (Frequency), or highest mean absolute Impact value first (Shap impact)", 
...
class InteractionSummaryComponent(ExplainerComponent):
    ...
    def __init__(self, explainer, title="Interactions Summary", name=None,
        ...
        if self.description is None: self.description = """
        Shows shap interaction values. Each Impact value can be decomposed into a direct
...
class ShapContributionsGraphComponent(ExplainerComponent):
    ...
    def __init__(self, explainer, title="Contributions Plot", name=None,
        ...
        Args:
            ...
            sort ({'abs', 'high-to-low', 'low-to-high', 'importance'}, optional): sorting of Impact values. 
                        Defaults to 'high-to-low'.
            orientation ({'vertical', 'horizontal'}, optional): orientation of bar chart.
                        Defaults to 'vertical'.
            higher_is_better (bool, optional): Color positive Impact values green and
                negative Impact values red, or the reverse. 
            description (str, optional): Tooltip to display when hover over
                component title. When None default text is shown. 
        """
        super().__init__(explainer, title, name)
        ...
        if self.description is None: self.description = """
        This plot shows the contribution that each individual column has had
...
class ShapContributionsTableComponent(ExplainerComponent):
    ...
    def __init__(self, explainer, title="Contributions Table", name=None,
                    ...
                    description=None, **kwargs):
        """Show Impact values contributions to prediction in a table component
        ...
        if self.description is None: self.description = """
        This tables shows the contribution that each individual column has had
        on the prediction for a specific observation. The contributions (starting
        from the population average) add up to the final prediction. This allows you
        to explain exactly how each individual prediction has been built up
        from all the individual ingredients in the model.
                ...
                dbc.Row([
                    ...
                    make_hideable(
                        dbc.Col([
                            ...
                            dbc.Tooltip("Sort the columns either by highest absolute impact (absolute), "
                                        "from most positive to most negative (high-to-low),"
                                        "from most negative to most positive (low-to-high),"
                                        "or by the global impact ordering (importance).",
