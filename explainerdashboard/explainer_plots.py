...
def plotly_dependence_plot(X_col, shap_values, interact_col=None, 
    ...
    and Impact values for col_name. Do higher values of col_name increase prediction
    or decrease them? Or some kind of U-shape or other?

    Args:
        X_col (pd.Series): pd.Series with column values.
        shap_values (np.ndarray): Impact values generated for X_col
    ...
    assert len(X_col) == len(shap_values), \
        f"Column(len={len(X_col)}) and Impact values(len={len(shap_values)}) and should have the same length!"
    if idxs is not None:
    ...
    layout = go.Layout(
            ...
            yaxis=dict(title=f"Impact value ({units})" if units !="" else "Impact value")
        )
...
def plotly_shap_violin_plot(X_col, shap_values, X_color_col=None, points=False, 
    ...
    """Generates a violin plot for displaying Impact value distributions for
    categorical features.

    Args:
        X_col (pd.DataFrame): dataframe of input rows
        shap_values (np.ndarray): Impact values generated for X
    ...
    if X_color_col is not None and interaction:
        ...
        fig.update_layout(title=f'Impact values for {X_col.name}<br>(colored by {X_color_col.name})')
    else:
        fig.update_layout(title=f'Impact values for {X_col.name}')
    fig.update_layout(margin=dict(t=40, b=40, l=40, r=40))
    return fig
...
def plotly_shap_scatter_plot(X, shap_values_df, display_columns=None, title="Impact values", 
                idxs=None, highlight_index=None, na_fill=-999, round=3, max_cat_colors=5):
    """Generate a Impact values summary plot where features are ranked from
    highest mean absolute Impact value to lowest, with point clouds shown
    for each feature. 
